...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --platforms LIST            Comma-separated list of platforms to build (default: all)"
            echo "                              Available: win-x64, win-arm64, osx-x64, osx-arm64, linux-x64, linux-arm64"
            echo "                              Example: --platforms win-x64,linux-x64"
            ...
            echo "  $0 --platforms win-x64,linux-x64             # Build only Windows and Linux x64"
            echo "  $0 --platforms osx-arm64                     # Build only macOS ARM64"
...
if [ -n "$SELECTED_PLATFORMS" ]; then
    ...
    if [ ${#FILTERED_BUILDS[@]} -eq 0 ]; then
        print_error "No valid platforms found in: $SELECTED_PLATFORMS"
        print_status "Available platforms: win-x64, win-arm64, osx-x64, osx-arm64, linux-x64, linux-arm64"
        exit 1
    fi
    
    BUILDS=("${FILTERED_BUILDS[@]}")
    print_status "Building ${#BUILDS[@]} selected platform(s)"
fi
